...
DhcpClient::StartApplication (void)
{
  ...
  m_remoteAddress = Ipv4Address ("255.255.255.255");   /// broadcast address
  m_myAddress = Ipv4Address ("0.0.0.0");   
  m_gateway = Ipv4Address ("0.0.0.0");
  Ptr<Ipv4> ipv4 = GetNode ()->GetObject<Ipv4> ();    /// get pointer assigned to that device(here dhcpclient)
  uint32_t ifIndex = ipv4->GetInterfaceForDevice (m_device);   //ifindex has ipv4 type interface address which contain value m_device 
  																//that is return netdevice for m_device
  ...
  Address myAddress = m_device->GetAddress ();    //myadress has m_device address  
  NS_LOG_INFO ("My address is " << myAddress);
  uint8_t addr[Address::MAX_SIZE];   //array of MAX_SIZE
  std::memset (addr, 0, Address::MAX_SIZE); /// addr set to zero
  uint32_t len = myAddress.CopyTo (addr);  // len contains length of myaddress and addr contains myaddress value
  NS_ASSERT_MSG (len <= 16, "DHCP client can not handle a chaddr larger than 16 bytes"); //conditipn fails display the message
  m_chaddr.CopyFrom (addr, 16);  /// m_chaddr copies value first 16 bit value from addr
  NS_LOG_INFO ("My m_chaddr is " << m_chaddr);

  bool found = false;
 // GetNAdress --> the number of Ipv4InterfaceAddresss stored on this interface 
  for (uint32_t i = 0; i < ipv4->GetNAddresses (ifIndex); i++)
    {
      if (ipv4->GetAddress (ifIndex, i).GetLocal () == m_myAddress)    ///checking any interface of this device is equal my_address value
        {
          found = true;
        }
    }
  if (!found)    /// if not add new interface and assign the m_myaddress value to that
    {
  ...
  m_socket->SetRecvCallback (MakeCallback (&DhcpClient::NetHandler, this));   // notify the application when new data is available to read make a call to NetHandler with this instance 

  m_device->AddLinkChangeCallback (MakeCallback (&DhcpClient::LinkStateHandler, this));  //when change in link state callback to LinkSateHandler
  Boot ();  // sends DHCP DISCOVER and client state to WAIT_OFFER
